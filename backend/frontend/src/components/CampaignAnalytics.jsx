...
const CampaignAnalytics = ({ filters = {} }) => {
  ...
  const setupRealtimeSubscriptions = () => {
    // Debounce timer to prevent too many refreshes during high-volume campaigns
    let debounceTimer = null;
    const debouncedFetchStats = () => {
      if (debounceTimer) clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => {
        fetchStats();
      }, 3000); // Refresh at most every 3 seconds for analytics
    };

    // Subscribe to campaigns, messages, and send_queue for real-time updates
    ...
    const sendQueueChannel = supabase
      .channel('sendqueue-analytics')
      .on(
        'postgres_changes',
        {
          event: '*',
